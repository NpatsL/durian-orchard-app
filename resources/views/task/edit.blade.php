<x-app-layout>
    <div class="max-w-2xl mx-auto mt-20 p-4 shadow-md rounded-lg border-t-2 border-teal-400">
        ...
        <form action="{{ route('task.update', $task->id) }}" method="POST"
            ...
            <div class="mb-4">
                ...
                <select id="material_id" name="material_id">
                    ...
                    @foreach ($materials->reverse() as $material)
                        <option value="{{ $material->id }}" {{ $task->material_id == $material->id ? 'selected' : '' }}>
                            ( {{ $material->lotMaterial->date }} ) {{ $material->name }} ( {{ $material->unit }} )
                        </option>
                    @endforeach
                </select>
            </div>
            ...
            <div class="mb-4">
                <label id="" for="use_qty" class="block text-gray-700 text-sm font-bold mb-2">Quantity used
                </label>
